...
export function handleAssistantStatus(event: Electron.IpcMainInvokeEvent, query?: AssistantDataQuery) {
    ...
    if (query?.kind === 'models:list') return devscopeCore.assistant.listModels(Boolean(query.forceRefresh))
...
export function handleAssistantGetSnapshot(event: Electron.IpcMainInvokeEvent) {
    subscribeSender(event)
    return devscopeCore.assistant.getSnapshot()
}
...
export function handleAssistantRespondUserInput(
    event: Electron.IpcMainInvokeEvent,
    requestId: number,
    answers: Record<string, string | string[]>
) {
    subscribeSender(event)
    log.info('IPC: assistant:respondUserInput', { requestId, answerCount: Object.keys(answers || {}).length })
    return devscopeCore.assistant.respondUserInput(Number(requestId), answers)
}
...
export async function handleAssistantListModels(
    ...
    return await devscopeCore.assistant.listModels(Boolean(options?.forceRefresh))
}
