...
function buildUserMessageDisplayText(prompt: string, options: AssistantSendOptions): string {
    ...
    if (trimmedPrompt) {
        const userPromptMatch = trimmedPrompt.match(/##\s+User Prompt\s+([\s\S]*?)(?:\n##\s+[^\n]+|$)/i)
        const extractedUserPrompt = userPromptMatch?.[1]?.trim()
        if (extractedUserPrompt) return extractedUserPrompt
    ...
    try {
        ...
        try {
            ...
            if (this.isThreadNotFoundError(error)) {
                ...
                turnStartResult = await this.requestWithRetry('turn/start', turnStartParams, { retries: 1 })
            } else if (selectedProjectPath && this.isInvalidParamsError(error)) {
                ...
                try {
                    ...
                    if (!this.isThreadNotFoundError(fallbackError)) {
                        throw fallbackError
                    }
                    resetDeadThread()
                    threadId = await this.ensureThread(resolvedModel, selectedProjectPath || undefined)
                    fallbackParams.threadId = threadId
                    turnStartResult = await this.requestWithRetry('turn/start', fallbackParams, { retries: 1 })
                }
            } else {
                if (!resolvedModel || !this.isMissingModelError(error)) {
                ...
                try {
                    ...
                    resetDeadThread()
                    threadId = await this.ensureThread(fallbackModel, selectedProjectPath || undefined)
                    turnStartParams.threadId = threadId
                    turnStartResult = await this.requestWithRetry('turn/start', turnStartParams, { retries: 1 })
                }
            }
