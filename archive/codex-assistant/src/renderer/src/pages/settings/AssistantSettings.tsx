...
export default function AssistantSettings() {
    ...
    const assistantController = useAssistantPageController()
    ...
    const loadModels = useCallback(async (forceRefresh = false) => {
        if (!forceRefresh && !assistantController.status.connected) {
            setModelsError(null)
    ...
    }, [assistantController.status.connected])
                ...
                {activeTab === 'connection' && (
                    <SettingsSection title="Connection" description="Turn assistant on and control startup behavior.">
                        <div className="space-y-4">
                            ...
                            />
                            <div className="flex items-start justify-between gap-4 rounded-xl border border-sparkle-border bg-black/10 px-4 py-3">
                                <div className="flex items-start gap-3">
                                    <PlugZap size={16} className="mt-0.5 text-cyan-300" />
                                    <div>
                                        <div className="text-sm font-medium text-sparkle-text">Auto-connect on Assistant page open</div>
                                        <div className="text-xs text-sparkle-text-muted mt-1">Always enabled. Opening Assistant will automatically try to connect.</div>
                                    </div>
                                </div>
                            ...
                            <div className="flex items-center justify-between gap-4 rounded-xl border border-sparkle-border bg-black/10 px-4 py-3">
                                <div>
                                    <div className="text-sm font-medium text-sparkle-text">Assistant runtime connection</div>
                                    <div className="text-xs text-sparkle-text-muted mt-1">
                                        {assistantController.connectionState === 'connected'
                                            ? 'Assistant is connected and ready.'
                                            : assistantController.connectionState === 'connecting'
                                                ? 'Assistant is connecting.'
                                                : assistantController.errorMessage || 'Assistant is disconnected.'}
                                    </div>
                                </div>
                                <button
                                    type="button"
                                    onClick={() => {
                                        if (assistantController.connectionState === 'connected') {
                                            void assistantController.handleDisconnect()
                                        } else {
                                            void assistantController.handleConnect()
                                        }
                                    }}
                                    disabled={assistantController.isConnecting}
                                    className={cn(
                                        'rounded-lg border transition-colors disabled:opacity-60 p-2',
                                        assistantController.connectionState === 'connected'
                                    ...
                                    title={assistantController.connectionState === 'connected' ? 'Disconnect assistant' : 'Connect assistant'}
                                >
                                    <PlugZap size={16} />
                                </button>
                            </div>
