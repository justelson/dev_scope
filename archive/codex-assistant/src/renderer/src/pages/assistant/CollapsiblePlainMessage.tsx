...
export function CollapsiblePlainMessage({
    ...
    const resolveAttachmentName = (attachment: AssistantHistoryAttachment, index: number): string => {
        ...
        const isPastedText = rawPath.startsWith('clipboard://') && (attachment.kind === 'doc' || attachment.kind === 'code' || attachment.textPreview)
        const isPastedImage = rawPath.startsWith('clipboard://') && attachment.kind === 'image'
        
        if (isPastedText) return 'Pasted text'
        if (isPastedImage) return 'Pasted image'
    ...
    return (
        <div ref={sectionRef} className={cn('assistant-t3-user-message group/usermsg max-w-full', isUser ? 'ml-auto w-auto' : 'w-full')}>
            <div className={cn(
                'assistant-t3-user-card relative max-w-[78ch] rounded-2xl rounded-br-sm border border-white/10 bg-white/[0.04] px-4 py-3 whitespace-pre-wrap break-words [overflow-wrap:anywhere] [word-break:break-word] text-sparkle-text',
                ...
                {visibleAttachments.length > 0 && (
                    <div className="mb-3 grid gap-2 sm:grid-cols-2">
                        {visibleAttachments.map((attachment, index) => {
                            ...
                            return (
                                <div key={`${attachment.path}-${name}-${index}`} className="overflow-hidden rounded-xl border border-white/10 bg-sparkle-bg/85">
                                    <div className="flex items-center justify-between gap-2 border-b border-white/5 px-2.5 py-1.5">
                                        <p className="truncate text-[11px] text-sparkle-text" title={name}>{name}</p>
                                        <span className="rounded-md border border-white/10 px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-sparkle-text-muted">
                                            {kind}
                                        </span>
                                    </div>
                ...
                <div
                    ref={contentRef}
                    className={cn('relative whitespace-pre-wrap break-words [overflow-wrap:anywhere] [word-break:break-word]')}
                    style={
                        showFade
                            ? {
                ...
                {isUser && (
                    <div className="mt-3 flex items-center justify-between gap-2 border-t border-white/5 pt-2">
                        ...
                        <button
                            ...
                            className={cn(
                                'group/copybtn inline-flex h-6 items-center overflow-hidden rounded-md border transition-all duration-300',
                                copied
                                    ? 'gap-1.5 border-emerald-500/40 bg-emerald-500/10 px-2 text-emerald-300'
                                    : 'gap-0 border-white/10 bg-sparkle-bg px-2 text-sparkle-text-secondary hover:gap-1.5 hover:bg-sparkle-card-hover hover:text-sparkle-text'
                            )}
                            title={copied ? 'Copied' : 'Copy message'}
                        >
                            {copied ? <Check size={11} /> : <Copy size={11} />}
                            <span className={cn(
                                'whitespace-nowrap text-[10px] transition-all duration-300',
                                copied ? 'w-auto opacity-100' : 'w-0 opacity-0 group-hover/copybtn:w-auto group-hover/copybtn:opacity-100'
                            )}>
                                {copied ? 'Copied' : 'Copy'}
                            </span>
                        </button>
            ...
            {showFloatingCollapse && canCollapse && !isCollapsed && (
                <button
                    ...
                    className="fixed z-40 inline-flex items-center gap-2 rounded-full border border-white/10 bg-sparkle-card px-4 py-2 text-sm text-sparkle-text shadow-lg transition-colors hover:bg-sparkle-card-hover"
                >
                    <ChevronUp size={14} />
                    Show Less
                </button>
