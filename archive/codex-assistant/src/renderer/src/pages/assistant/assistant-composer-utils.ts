...
export function shouldShowPastedTextFormattingWarning(file: ComposerContextFile | null | undefined): boolean {
    if (!file || !isPastedTextAttachment(file)) return false

    const content = String(file.content || '')
    if (!content) return false

    return content.endsWith('\n\n[truncated]') || content.includes('\uFFFD') || content.includes('\u0000')
}
