...
export function basenameOfPath(pathValue: string): string {
    ...
    if (slashIndex === -1) return normalizedPath
    return normalizedPath.slice(slashIndex + 1)
}
...
function resolveLanguageFallbackDefinition(pathValue: string, theme: 'light' | 'dark'): string | null {
    ...
    for (const candidate of extensionCandidates(basename)) {
        const languageId =
            localLanguageIdByExtensionOverrides[candidate as keyof typeof localLanguageIdByExtensionOverrides] ?? languageIdByExtension[candidate]
...
export function getVscodeIconUrlForEntry(
    ...
    const definitionKey = kind === 'directory'
        ? resolveFolderDefinition(pathValue, theme)
        : resolveFileDefinition(pathValue, theme)
    const iconFilename = iconFilenameForDefinitionKey(definitionKey)
