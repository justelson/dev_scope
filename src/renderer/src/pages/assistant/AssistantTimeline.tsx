...
function getTimelineEntries(messages: AssistantMessage[], activities: AssistantActivity[]): TimelineEntry[] {
    const allEntries = [
...
function getActivityElapsed(activity: AssistantActivity): string | null {
    ...
    const durationMs = typeof durationCandidate === 'number'
        ? durationCandidate
    ...
    if (startedAt && completedAt) {
        return formatWorkingTimer(startedAt, completedAt)
    }
...
const TimelineToolCallCard = memo(({ activity }: { activity: AssistantActivity }) => {
    ...
    const copyValue = useMemo(
        () => [primaryValue, output, ...detailLines].filter((value) => String(value || '').trim()).join('\n\n'),
    ...
    return (
        <div className="px-2 py-1.5">
            <button
                ...
                className="flex w-full min-w-0 items-center gap-2 overflow-hidden rounded-lg text-left transition-colors hover:bg-white/[0.02]"
            >
                <span
                    className={cn(
                        ...
                        status === 'success'
                            ...
                            : status === 'running'
                                ? 'border-amber-400/20 bg-amber-500/[0.10] text-amber-300'
                ...
                <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-[9px] font-medium uppercase tracking-[0.14em] text-white/20">
                        {title}
                        {elapsed ? <span className="ml-1.5 normal-case tracking-normal text-white/22">• {elapsed}</span> : null}
                    </p>
                </div>
                ...
                <div className="mt-2 rounded-lg border border-white/5 bg-black/20 p-2.5">
                    <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                            <p className="text-[10px] text-white/18">
                                {formatAssistantDateTime(activity.createdAt)}
                                {elapsed ? <span className="ml-1.5">• {elapsed}</span> : null}
                            </p>
                    ...
                    {output && (
                        <div className="mt-2 rounded-md border border-white/5 bg-black/25 p-2">
                            ...
                            <p className="mt-1 whitespace-pre-wrap break-all font-mono text-[11px] leading-5 text-white/22">{output}</p>
                        </div>
                    )}
                    {detailLines.map((line, index) => (
                        <p key={`${activity.id}-${index}`} className="mt-1.5 whitespace-pre-wrap break-all font-mono text-[11px] leading-5 text-white/18">
                            {line}
                        </p>
                    ))}
                </div>
...
const TimelineToolCallList = memo(({ activities }: { activities: AssistantActivity[] }) => {
    const [expanded, setExpanded] = useState(false)
    ...
    const visibleActivities = useMemo(() => {
        if (expanded || !hasMore) return activities
        return activities.slice(-5)
    }, [activities, expanded, hasMore])

    return (
        <div className="max-w-4xl py-2">
            <div className="overflow-hidden rounded-xl border border-white/10 bg-white/[0.03]">
                <div className="flex items-center justify-between gap-2 px-2 pt-1.5 pb-0">
                    <div className="text-[9px] font-medium uppercase tracking-[0.22em] text-white/20">
                        {header}
                    </div>
                    {hasMore && (
                        <button
                ...
                <div>
                    {visibleActivities.map((activity) => (
                        <div key={activity.id}>
                            <TimelineToolCallCard activity={activity} />
                        </div>
                    ))}
                </div>
            </div>
...
const TimelineMessage = memo(({
    ...
    const assistantElapsed = useMemo(() => {
        if (!isAssistant || !isLatestAssistant || !latestTurnStartedAt) return null
            ...
            <div className="max-w-4xl py-1">
                ...
                <div className="mt-3">
                    <p className="text-[11px] text-sparkle-text-muted">
                        {formatAssistantDateTime(message.updatedAt)}
                        {assistantElapsed ? <span className="ml-1.5">• {assistantElapsed}</span> : null}
                    </p>
                    {message.streaming && (
                        <span className="inline-flex items-center gap-1 text-[11px] text-sparkle-text-secondary">
                ...
                <div className="mt-2 flex items-center justify-between gap-3 border-t border-white/5 pt-2">
                    ...
                    <div className="flex items-center gap-1 opacity-0 transition-all group-hover:opacity-100">
                        <button
                            ...
                            className={cn(
                                'rounded-md border p-1 transition-all',
                                copied
                                    ? 'border-emerald-400/20 bg-emerald-500/[0.08] text-emerald-300'
                                    : 'border-white/10 bg-white/[0.03] text-sparkle-text-muted hover:border-white/20 hover:text-sparkle-text'
                            )}
                        ...
                        {onRequestDelete && (
                            <button
                                ...
                                className={cn(
                                    'rounded-md border p-1 transition-all',
                                    deleting
                                        ? 'cursor-not-allowed border-red-400/20 bg-red-500/[0.08] text-red-200/70'
                                        : 'border-white/10 bg-white/[0.03] text-sparkle-text-muted hover:border-red-400/20 hover:bg-red-500/[0.08] hover:text-red-200'
                                )}
...
function TimelineWorkingIndicator({ startedAt }: { startedAt?: string | null }) {
    const [nowIso, setNowIso] = useState(() => new Date().toISOString())
        ...
        return (
            <div className="flex h-full min-h-[420px] items-center justify-center rounded-xl border border-dashed border-white/8 bg-white/[0.015]">
                <div className="max-w-md px-6 text-center">
                    ...
                    <p className="mt-3 text-sm leading-6 text-sparkle-text-secondary">
                        Ask for a concrete change and the assistant will stream replies here alongside command, file, and tool actions.
                    </p>
                </div>
            </div>
        )
