...
import {
    ...
    bridgeUpsertStreamingAssistantMessage
...
type PendingUserInputRequest = AssistantPendingUserInput & {
    requestId: number
}
...
export class AssistantBridge extends EventEmitter {
    ...
    private pendingUserInputRequests = new Map<number, PendingUserInputRequest>()
    ...
    public respondToUserInput(requestId: number, answers?: Record<string, string | string[]>) {
        return bridgeRespondToUserInput(this, requestId, answers)
    }
    public getHistory() {
        return bridgeGetHistory(this)
    }
    public getSnapshot() {
        return bridgeGetSnapshot(this)
    }
    public clearHistory() {
    ...
    private upsertStreamingAssistantMessage(turnId: string, attemptGroupId: string, text: string) {
        bridgeUpsertStreamingAssistantMessage(this, turnId, attemptGroupId, text)
    }
    private commitAssistantMessage(turnId: string, attemptGroupId: string, text: string) {
        bridgeCommitAssistantMessage(this, turnId, attemptGroupId, text)
    }
    ...
    private isThreadNotFoundError(error: unknown) {
        return bridgeIsThreadNotFoundError(error)
    }
