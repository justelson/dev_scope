...
export function bridgeHandleReasoningNotification(
    ...
    if (normalizedMethod.includes('summarytextdelta')) {
        return false
    }
...
export function bridgeHandleLegacyReasoningNotification(
    ...
    if (normalizedType.includes('summarytextdelta')) {
        return false
    }
...
export function bridgeNormalizeActivity(
    ...
    if (target.includes('file')) kind = 'file'
    else if (target.includes('search') || target.includes('web')) kind = 'search'
    else if (target.includes('command') || target.includes('exec')) kind = 'tool'
    else if (target.includes('tool')) kind = 'tool'

    if (kind === 'other') return null

    const summaryCandidates = kind === 'file'
        ? [payload.summary, payload.path, payload.filePath, payload.name]
        : kind === 'search'
            ? [payload.summary, payload.query, payload.pattern, payload.url]
            : [payload.summary, payload.command, payload.tool, payload.name, payload.description]

    const summary = formatActivitySummary(
        summaryCandidates.map((value) => readString(value)).find((value) => value.trim()) || ''
    )
...
export function bridgeEmitActivity(
    ...
    const burstKey = `${kind}::${normalizeToken(method) || method}::${normalizeToken(normalizedSummary)}`
...
export function bridgeEmitEvent(
    ...
    const event: AssistantEventPayload = {
        sequence: (Number(bridge.snapshotSequence) || 0) + 1,
        type,
        timestamp: now(),
        payload
    }
    bridge.snapshotSequence = event.sequence
