...
function setActiveAttemptForGroup(
    bridge: BridgeCoreContext,
    attemptGroupId: string,
    activeMessageId: string | null
): void {
    for (const message of bridge.history) {
        if (message.role !== 'assistant' || message.attemptGroupId !== attemptGroupId) continue
        message.isActiveAttempt = Boolean(activeMessageId) && message.id === activeMessageId
    }
}

export function bridgeUpsertStreamingAssistantMessage(
    bridge: BridgeCoreContext,
    turnId: string,
    attemptGroupId: string,
    text: string
    ...
    let message = buffer.messageId
        ? bridge.history.find((entry: AssistantHistoryMessage) => entry.id === buffer.messageId)
        : null

    if (!message) {
        const existingAttemptIndexes = bridge.history
            .filter((entry: AssistantHistoryMessage) => entry.role === 'assistant' && entry.attemptGroupId === attemptGroupId)
            .map((entry: AssistantHistoryMessage) => entry.attemptIndex || 0)
        ...
        bridge.history.push(message)
        buffer.messageId = message.id
        buffer.messageCreatedAt = message.createdAt
    } else {
        message.text = normalizedText
        message.turnId = turnId
        message.attemptGroupId = attemptGroupId
        message.isActiveAttempt = true
        message.streaming = true
    }

    bridge.turnAttemptGroupByTurnId.set(turnId, attemptGroupId)
    setActiveAttemptForGroup(bridge, attemptGroupId, message.id)
    bridge.emitEvent('history', { history: [...bridge.history] })
}

export function bridgeCommitAssistantMessage(
    bridge: BridgeCoreContext,
    turnId: string,
    attemptGroupId: string,
    text: string
    ...
    bridgeUpsertStreamingAssistantMessage(bridge, turnId, attemptGroupId, normalizedText)

    const message = buffer.messageId
        ? bridge.history.find((entry: AssistantHistoryMessage) => entry.id === buffer.messageId)
        : null
    if (!message) return
    ...
    buffer.draft = ''
    buffer.draftKind = null
    buffer.pendingFinal = null
    buffer.pendingFinalPhase = null
    buffer.messageId = null
    buffer.messageCreatedAt = null
}
...
export function bridgeFinalizeTurn(
    ...
    const finalText = shouldLockFinal
        ...
        : ''
    const existingStreamingMessage = buffer.messageId
        ? bridge.history.find((message: AssistantHistoryMessage) => message.id === buffer.messageId)
        : null

    if (shouldLockFinal && finalText) {
        const persistedReasoningText = composeReasoningText(bridge.reasoningTextsByTurn.get(turnId) || [])
        let assistantMessage = existingStreamingMessage
        ...
        if (!assistantMessage) {
            ...
            buffer.messageId = assistantMessage.id
            buffer.messageCreatedAt = assistantMessage.createdAt
        }

        assistantMessage.text = finalText
        assistantMessage.reasoningText = persistedReasoningText || undefined
        assistantMessage.turnId = turnId
        assistantMessage.attemptGroupId = attemptGroupId
        assistantMessage.isActiveAttempt = true
        assistantMessage.streaming = false
        bridge.turnAttemptGroupByTurnId.set(turnId, attemptGroupId)
        setActiveAttemptForGroup(bridge, attemptGroupId, assistantMessage.id)

        bridge.emitEvent('assistant-final', {
            turnId,
            text: finalText,
            attemptGroupId,
            attemptIndex: assistantMessage.attemptIndex || 1,
        ...
        bridge.emitEvent('history', { history: [...bridge.history] })
    } else if (existingStreamingMessage) {
        existingStreamingMessage.streaming = false
        existingStreamingMessage.text = finalText || fallbackDraft || existingStreamingMessage.text
        setActiveAttemptForGroup(bridge, attemptGroupId, existingStreamingMessage.id)
        bridge.emitEvent('history', { history: [...bridge.history] })
    }
    ...
    for (const pending of Array.from(bridge.pendingUserInputRequests.values() as Iterable<any>)) {
        if (pending.turnId !== turnId) continue
        bridge.pendingUserInputRequests.delete(pending.requestId)
    }
