...
export function bridgeGetHistory(bridge: BridgeSessionContext): {
    ...
    return {
        ...
        pendingUserInputs: Array.from(bridge.pendingUserInputRequests.values()).map((entry: any) => ({
            requestId: Number(entry.requestId),
            questions: Array.isArray(entry.questions) ? entry.questions.map((question: any) => ({
            ...
            turnId: entry.turnId ? String(entry.turnId) : null,
            attemptGroupId: entry.attemptGroupId ? String(entry.attemptGroupId) : null,
            createdAt: Number(entry.timestamp || entry.createdAt) || now()
        }))
    }
}
...
function findLatestEventByType(
    ...
    type: string
): AssistantEventPayload | null {
    for (const event of bridge.eventStore as AssistantEventPayload[]) {
        if (event.type === type) return event
    }
    return null
}

export function bridgeGetSnapshot(bridge: BridgeSessionContext): {
    success: boolean
    ...
    lastRateLimitsUpdateAt: number | null
} {
    const historySnapshot = bridgeGetHistory(bridge)
    const telemetryIntegrity = bridgeGetTelemetryIntegrity(bridge)
    const tokenUsageEvent = findLatestEventByType(bridge, 'thread/tokenUsage/updated')
    const tokenPayload = tokenUsageEvent?.payload && typeof tokenUsageEvent.payload === 'object'
        ? tokenUsageEvent.payload as Record<string, unknown>
        : null
    const accountUpdatedEvent = findLatestEventByType(bridge, 'account/updated')
    const rateLimitsUpdatedEvent = findLatestEventByType(bridge, 'account/rateLimits/updated')

    return {
        success: true,
        snapshotSequence: Number(bridge.snapshotSequence) || 0,
        status: bridge.getStatus(),
        history: historySnapshot.history,
        activeSessionId: historySnapshot.activeSessionId,
        sessions: historySnapshot.sessions,
        partsByTurn: historySnapshot.partsByTurn,
        pendingApprovals: historySnapshot.pendingApprovals,
        pendingUserInputs: historySnapshot.pendingUserInputs,
        telemetryIntegrity: {
            eventsStored: telemetryIntegrity.eventsStored,
            monotonicDescending: telemetryIntegrity.monotonicDescending,
            newestTimestamp: telemetryIntegrity.newestTimestamp,
            oldestTimestamp: telemetryIntegrity.oldestTimestamp
        },
        threadTokenUsage: tokenUsageEvent && tokenPayload
            ? {
                threadId: typeof tokenPayload.threadId === 'string' ? tokenPayload.threadId : undefined,
                model: typeof tokenPayload.model === 'string' ? tokenPayload.model : undefined,
                inputTokens: Number(tokenPayload.inputTokens) || undefined,
                outputTokens: Number(tokenPayload.outputTokens) || undefined,
                totalTokens: Number(tokenPayload.totalTokens) || undefined,
                modelContextWindow: Number(tokenPayload.modelContextWindow) || undefined,
                at: tokenUsageEvent.timestamp
            }
            : null,
        lastAccountUpdateAt: accountUpdatedEvent?.timestamp ?? null,
        lastRateLimitsUpdateAt: rateLimitsUpdatedEvent?.timestamp ?? null
    }
}
export function bridgeClearHistory(bridge: BridgeSessionContext): { success: boolean } {
    ...
    bridge.pendingUserInputRequests.clear()
