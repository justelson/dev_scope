...
import type { AssistantApprovalDecision, AssistantPendingUserInput, AssistantUserInputQuestion } from './types'
...
function normalizeUserInputQuestions(value: unknown): AssistantUserInputQuestion[] {
    ...
    return value
        .map((entry) => {
            ...
            const header = readString(question.header).trim()
            const prompt = readString(question.question).trim()
            ...
            if (!id || !header || !prompt || options.length === 0) return null
            return {
                id,
                header,
                question: prompt,
...
function extractLegacyAgentDeltaText(payload: Record<string, unknown>): string {
    ...
    const nestedCandidates = [
        readRecord(payload.delta),
        readRecord(payload.message),
        readRecord(payload.content),
        readRecord(payload.item),
        readRecord(payload.part)
    ]
...
export function bridgeHandleServerRequest(
    ...
    if (method === 'item/tool/requestUserInput') {
        const requestPayload = readRecord(message.params) || {}
        const questions = normalizeUserInputQuestions(requestPayload.questions)
        if (questions.length === 0) {
            bridge.send({
                id,
                error: {
                    code: -32602,
                    message: 'Unsupported user input request payload.'
                }
            })
            return
        }
        ...
        const attemptGroupId = turnId
            ? (bridge.turnContexts.get(turnId)?.attemptGroupId
                ...
                || turnId)
            : null
        const pendingRequest: AssistantPendingUserInput = {
            requestId: id,
            questions,
            timestamp: now(),
        ...
        bridge.pendingUserInputRequests.set(id, pendingRequest)
        bridge.emitEvent('user-input-request', {
            requestId: id,
            questions,
            turnId,
            attemptGroupId
...
export function bridgeRespondToUserInput(
    bridge: BridgeRpcContext,
    requestIdRaw: number,
    answersRaw?: Record<string, string | string[]>
    ...
    const pending = bridge.pendingUserInputRequests.get(requestId)
    if (!pending) {
        return { success: false, error: `No pending user input request found: ${requestId}` }
    }

    const answers = normalizeUserInputAnswers(answersRaw)
    if (!answers) {
        return { success: false, error: 'answers must be a record of non-empty strings or string arrays.' }
    }
    if (!bridge.proc || !bridge.proc.stdin?.writable) {
        return { success: false, error: 'Assistant bridge is not connected.' }
    }

    bridge.pendingUserInputRequests.delete(requestId)
    bridge.send({ id: requestId, result: { answers } })
    bridge.emitEvent('user-input-response', {
        requestId,
        answers,
        turnId: pending.turnId || null,
        attemptGroupId: pending.attemptGroupId || null
    })
    if (pending.turnId) {
        bridge.recordTurnPart({
            turnId: pending.turnId,
...
export function bridgeHandleNotification(
    ...
    if (method === 'item/agentMessage/delta') {
        ...
        if (isProvisional && updateText) {
            ...
            bridge.upsertStreamingAssistantMessage(turnId, attemptGroupId, buffer.draft)
            return
        }
    ...
    if (method === 'item/completed') {
        ...
        if (isFinalAnswerPhase(phase)) {
            ...
            bridge.commitAssistantMessage(turnId, attemptGroupId, text)
        }
        return
    }
...
export function bridgeHandleLegacyNotification(
    ...
    if (eventType === 'agent_message_delta' || eventType === 'agent_message_content_delta') {
        const text = extractLegacyAgentDeltaText(payload)
        if (!text) return
        ...
        const attemptGroupId = bridge.turnContexts.get(turnId)?.attemptGroupId
            || bridge.turnAttemptGroupByTurnId.get(turnId)
            || turnId
        buffer.draft = mergeAgentMessageDraft(buffer.draft, text, {
            preferReplaceForDisjointLong: true
        })
        buffer.draftKind = 'provisional'
        bridge.emitEvent('assistant-delta', {
