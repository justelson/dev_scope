...
function isWorkLikeRole(value: unknown): boolean {
    const normalized = normalizeToken(readString(value))
...
function isWorkLikeItem(item: Record<string, unknown>): boolean {
    ...
    if (
        ...
        || itemType.includes('approval')
        || itemType.includes('input')
        || itemType.includes('function')
    ) {
        return true
    }

    return isWorkLikeRole(item.role) || isWorkLikeRole(item.author)
}
...
export function extractCompletedAgentText(params: Record<string, unknown>): string {
    ...
    const messageText = readString(item.message).trim()
    if (messageText) return messageText

    if (isAssistantMessageItem(item)) return ''

    return readTextFromContent(params.text || params.message || params.content).trim()
}
