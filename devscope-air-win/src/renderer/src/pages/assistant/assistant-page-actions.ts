...
export function createAssistantPageActions(ctx: ActionContext) {
    ...
    const handleSend = async (prompt: string, contextFiles: ComposerContextFile[]): Promise<boolean> => {
        ...
        try {
            ...
            const hasAssistantReply = ctx.history.some((message) => message.role === 'assistant')
            const titleLooksAutoGenerated = /^session\s+\d+$/i.test(ctx.activeSessionTitle) || /^untitled session$/i.test(ctx.activeSessionTitle)
            const normalizedTitle = ctx.activeSessionTitle.replace(/\s+/g, ' ').trim()
    ...
    const handleCreateSession = async () => {
        ...
        const createdSessionId = String(created?.session?.id || '').trim()
        if (createdSessionId) {
            await window.devscope.assistant.setSessionProjectPath(createdSessionId, ctx.settings.projectsFolder.trim())
        }
