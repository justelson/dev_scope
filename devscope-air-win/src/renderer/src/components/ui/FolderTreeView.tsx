...
export default function FolderTreeView({
    ...
    const parsedSearch = useMemo(() => parseFileSearchQuery(deferredSearchTerm), [deferredSearchTerm])
    const hasSearchTerm = deferredSearchTerm.trim().length > 0
    const hasRawSearchTerm = searchTerm.trim().length > 0
    const searchIndex = useMemo(() => buildFileSearchIndex(tree), [tree])
    const indexedSearch = useMemo(() => {
        if (!hasSearchTerm) return null
        return searchFileIndex(searchIndex, parsedSearch, {
            showHidden: hiddenVisible,
            includeDirectories: true
        })
    }, [hasSearchTerm, searchIndex, parsedSearch, hiddenVisible])
    ...
    // Filter tree using path sets from indexed search.
    const filterTree = useCallback((nodes: FileTreeNode[]): FileTreeNode[] => {
        const visiblePathSet = indexedSearch?.visiblePathSet
        return nodes
            .filter(node => {
                if (!hiddenVisible && node.isHidden) return false
                if (!hasSearchTerm) return true
                return Boolean(visiblePathSet?.has(node.path))
            })
            .map(node => {
                if (node.children) {
                    return {
    ...
    }, [hiddenVisible, hasSearchTerm, indexedSearch])

    const filteredTree = filterTree(tree)
    const isSearching = hasRawSearchTerm
