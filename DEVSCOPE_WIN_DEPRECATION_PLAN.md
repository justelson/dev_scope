...
Deprecate `devscope-win` as the primary product track and move all active development to the main DevScope desktop app at repository root.

`devscope-win` will enter maintenance-only mode first, then be archived after a defined support window.

## Why This Plan

1. The main DevScope desktop app is already the active path in repo docs and current development flow.
2. Recent feature velocity is concentrated in the root desktop app.
...
4. March 8, 2026: Remove `devscope-win` from the working tree while preserving it in git history.
...
2. Add migration note that the repository root desktop app is the only active path.
...
3. Keep all new roadmap work in the repository root desktop app.
...
1. Mark `devscope-win` as archived in docs and repository notes.
...
1. All default docs/onboarding point to the repository root desktop app.
...
4. Archive status is visible in root docs and repository notes.
