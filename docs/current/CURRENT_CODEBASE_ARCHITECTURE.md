...
Last validated against code on May 15, 2026.
...
The renderer also includes a dedicated `/quick-open` preview route for file-association and shell file launches. That route bypasses the main app shell and renders inside its own frameless preview window with renderer-owned chrome instead of native Electron frame chrome. The quick-preview route lazy-loads the shared `FilePreviewModal`, uses a minimal title bar with window controls, and disables fullscreen because the route is already hosted in a dedicated preview window.
...
May 15 verification note: current route, IPC, assistant, file-preview, Git, and update boundaries still match the May 14 architecture snapshot. No desktop app commits landed on `dev` after `e9c00d7`; the release/update helper surface remains on package version `1.6.6-beta.1`, GitHub Actions generates release bodies through `scripts/maint/generate-release-notes.mjs`, quick-preview chrome is intentionally minimal, and preview analysis work is gated behind expanded/visible inspector state to avoid unnecessary file-content scans.
