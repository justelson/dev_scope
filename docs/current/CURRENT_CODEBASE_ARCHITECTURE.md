...
Last validated against code on May 9, 2026.
...
May 9 verification note: current route, IPC, assistant, file-preview, Git, and update boundaries still match the May 6 architecture snapshot. No desktop app commits landed on `dev` after `cd7353f`; the release/update helper surface remains on package version `1.6.6-beta.1`, GitHub Actions generates release bodies through `scripts/maint/generate-release-notes.mjs`, quick-preview chrome is intentionally minimal, and preview analysis work is gated behind expanded/visible inspector state to avoid unnecessary file-content scans.
