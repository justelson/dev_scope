...
- session title generation, selected-session deletion fallback, assistant message/thread lifecycle, and streaming tool-output activity merges
...
Current assistant capabilities include session lifecycle, model listing, connect/disconnect, prompt send, interrupt, approval responses, user-input responses, project-path association, Playground root and lab setup, explicit session hydration, session title generation, connection recovery, selected-session deletion fallback, attachment persistence, event subscription, and streaming command/file-change activity updates.
