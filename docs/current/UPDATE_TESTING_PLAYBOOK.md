...
- GitHub Actions release bodies come from `scripts/maint/generate-release-notes.mjs`, not GitHub autogenerated notes
- real release only after those pass

## May 4 Verification Note

Updater runtime behavior did not change in the `1.6.6-beta.1` quick-preview release. The release workflow did change: GitHub Actions now publishes the repo-generated release body, while the packaged-update smoke path remains the local feed from `dist/releases/v<version>/`, with required validation for `latest.yml`, the installer `.exe`, and the installer `.exe.blockmap` before serving.
