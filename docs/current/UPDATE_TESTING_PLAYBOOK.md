...
Updater runtime behavior remains unchanged after the May 4 `1.6.6-beta.1` quick-preview release. GitHub Actions publishes the repo-generated release body, while the packaged-update smoke path remains the local feed from `dist/releases/v<version>/`, with required validation for `latest.yml`, the installer `.exe`, and the installer `.exe.blockmap` before serving.
