...
Updater runtime behavior remains unchanged after the May 9 docs refresh. GitHub Actions publishes the repo-generated release body, while the packaged-update smoke path remains the local feed from `dist/releases/v<version>/`, with required validation for `latest.yml`, the installer `.exe`, and the installer `.exe.blockmap` before serving.
