...
- Quick-preview shell/file-association route with lazy-loaded preview modal, minimal renderer-owned title bar, close/minimize/maximize controls, and fullscreen disabled inside the dedicated preview window: `Implemented`
...
- File-preview text analysis avoids unnecessary work by counting lines without splitting whole files and by building outlines/inspector diagnostics only when the expanded preview and right inspector state need them: `Implemented`
...
- GitHub Actions release workflow uses full git history, derives human release names from SemVer prerelease labels, and publishes the body generated by `scripts/maint/generate-release-notes.mjs` instead of GitHub's autogenerated notes: `Implemented`
...
## May 14 Current-State Verification

- Current package line is `1.6.6-beta.1`.
- No desktop app commits landed on `dev` after the May 13 docs refresh commit `918b6ef`.
- The quick-preview route now keeps its title bar minimal, lazy-loads the shared file-preview modal, and disables fullscreen because the preview already runs in a dedicated window.
- File-preview analysis work is lighter for large text files: line-limit checks count newlines directly, and outlines/inspector diagnostics are built only when the expanded preview and right inspector state require them.
- The release workflow now checks out full history and publishes release notes generated by `scripts/maint/generate-release-notes.mjs`, keeping GitHub Actions release bodies aligned with local release tooling.
