...
The tag-triggered release workflow should:

1. check out full history with `fetch-depth: 0`
2. derive the release title from the package/tag SemVer display label
3. run `node scripts/maint/generate-release-notes.mjs > release-notes.md`
4. pass `body_path: release-notes.md` to `softprops/action-gh-release`
5. disable GitHub autogenerated notes so the remote release body matches local release tooling
...
## May 4 Verification Note

Release-operation behavior now includes the GitHub Actions generated-notes path. The current package line is `1.6.6-beta.1`, with versioned output under `dist/releases/v<package-version>/`, generated release notes, local GitHub release publishing, and local packaged update-feed testing.
